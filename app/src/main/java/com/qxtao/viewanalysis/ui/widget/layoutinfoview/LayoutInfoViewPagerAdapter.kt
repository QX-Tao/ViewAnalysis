...
import com.qxtao.viewanalysis.ui.widget.boundsinfoview.BoundsInfoView
import com.qxtao.viewanalysis.ui.widget.layoutinfoview.infopage.InfoAdapter
import com.qxtao.viewanalysis.ui.widget.layoutinfoview.infopage.ItemInfo
...
class LayoutInfoViewPagerAdapter(
    ...
    private fun setInfo() = with(hierarchyNode) {
        ...
        list.add(ItemInfo("Activity", activity))
        list.add(ItemInfo("Widget", widget))
        list.add(
            ItemInfo(
                "ResourceId",
               resourceId.replace(packageName, "app").takeIf { it.isNotEmpty() } ?: "[NO_ID]"
            )
        )
        if (text.isNotEmpty()) {
            list.add(ItemInfo("Text", text))
        }
        list.add(ItemInfo("Important", isImportantForAccessibility))
